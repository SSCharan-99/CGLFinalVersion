...
public class UserInterface {
	...
	public void populateScreen(Pane p, String s) {
		System.out.println("Test case 0");
		StringBuffer sb = new StringBuffer();
		if (s.equals("oddCanvas")) {
			for (int r = 0; r < oddBoard.bW; r++) {
				for (int c = 0; c < oddBoard.bH; c++) {
					if (oddBoard.getBoard()[r][c].isLive()) {
						...
						for (int r1 = 0; r1 < evenBoard.bW; r1++) {
							for (int c1 = 0; c1 < evenBoard.bH; c1++) {
								if (evenBoard.getBoard()[r1][c1].isLive()) {
									Rectangle rect = new Rectangle(5, 5, Color.BLACK);
									rect.relocate(r1 * 5, c1 * 5);			
...
//	}
	...
	private void loadImageData() {
		...
		try {
			...
			evenBoard = new Board(boardSizeWidth, boardSizeHeight, oddBoard.getBoard());
		...
		populateScreen(oddCanvas, "oddCanvas");
	...
	public void runSimulation(){
		...
		if(toggle) {
			System.out.println("Test case run1222");
			window.getChildren().remove(oddCanvas);
			System.out.println("Test case run11");
			evenCanvas = new Pane();
			populateScreen(evenCanvas, "evenCanvas");
			System.out.println("Test case run2321");
			...
			toggle = false;
			System.out.println("Test case run1");
			}
		else {
			evenBoard.nextGeneration();
			evenBoard.printBoard();
			window.getChildren().remove(evenCanvas);
			oddCanvas = new Pane();
			populateScreen(oddCanvas, "oddCanvas");
			window.getChildren().add(oddCanvas);
			toggle = true;
			System.out.println("Test case run2");
			oddBoard.nextGeneration();
			oddBoard.printBoard();
		}
